...
function App() {
  ...
  const handleKeyDown = (event: KeyboardEvent): void => {
    ...
    if (newDirection !== currentDirection) return setCurrentDirection(newDirection)
  ...
  const handleControlClick = (direction: Direction) => () => {
    direction === currentDirection ? moveRobot() : setCurrentDirection(direction)
  }

  const handleCellClick = (event: React.MouseEvent<HTMLDivElement, MouseEvent>): void => {
  ...
  return (
    <Container>
      <span>You can also control the spider with your arrow keys</span>
      <Grid
        rows={5}
        cols={5}
